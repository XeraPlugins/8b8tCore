...
public class SeenCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            sendPrefixedLocalizedMessage(fromOn, "seen_syntax");
            return true;
        }
        if (args.length > 1) {
            ...
            if (to != fromOff) {
                if (to.hasPlayedBefore()) {
                    if (to.getPlayer() == null) {
                        Date seen = seen(fromOff);
                        sendPrefixedLocalizedMessage(fromOn, "seen", toName[0]);
                        sendMessage(sender, "   " + seen);
                        return true;
                    }
                    sendPrefixedLocalizedMessage(fromOn, "seen_online", toName[0]);
                    return true;
                }
                sendPrefixedLocalizedMessage(fromOn, "never_joined");
                return true;
            }
            sendPrefixedLocalizedMessage(fromOn, "seen_self");
            return true;
        }
    ...
    public Date seen(OfflinePlayer to) {
        return new Date(to.getLastSeen());
    }
